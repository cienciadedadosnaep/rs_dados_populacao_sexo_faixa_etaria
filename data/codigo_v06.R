...
dados %<>% mutate(`População masculina total (pessoas) (IBGE)`=round(`População masculina total (pessoas) (IBGE)`/1000000,2))
...
data_serie <- paste('[',gsub(' ',',',
                             paste(paste(as.vector(objeto_0[[1]]$`q3`)),
                                   collapse = ' ')),']',sep = '')
